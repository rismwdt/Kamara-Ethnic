                ...
                <div class="w-1/2 space-y-4">
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                      value="{{ old('name', $performer->name) }}" required autofocus />
                    ...
                    <div>
                        ...
                        <select id="gender" name="gender" class="mt-1 block w-full rounded border-gray-300 shadow-sm" required>
                            <option value="laki-laki" {{ old('gender', $performer->gender) === 'laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                            <option value="perempuan" {{ old('gender', $performer->gender) === 'perempuan' ? 'selected' : '' }}>Perempuan</option>
                            <option value="lainnya" {{ old('gender', $performer->gender) === 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        ...
                        <select id="performer_role_id" name="performer_role_id" class="mt-1 block w-full rounded border-gray-300 shadow-sm" required>
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}"
                                    {{ (string)old('performer_role_id', (string)$performer->performer_role_id) === (string)$role->id ? 'selected' : '' }}>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="w-1/2 space-y-4">
                    <div>
                        ...
                        <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full"
                                      value="{{ old('phone', $performer->phone) }}" required />
                    ...
                    <div>
                        ...
                        <select id="is_active" name="is_active" class="mt-1 block w-full rounded border-gray-300 shadow-sm" required>
                            @php $isActiveOld = (string)old('is_active', (string)($performer->is_active ? 1 : 0)); @endphp
                            <option value="1" {{ $isActiveOld === '1' ? 'selected' : '' }}>Ya</option>
                            <option value="0" {{ $isActiveOld === '0' ? 'selected' : '' }}>Tidak</option>
                        </select>
                        <x-input-error :messages="$errors->get('is_active')" class="mt-2" />
                    </div>

                    <div>
                        {{-- Checkbox Talent Eksternal --}}
                        <input type="hidden" name="is_external" value="0">
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="is_external" value="1"
                                   {{ old('is_external', $performer->is_external) ? 'checked' : '' }}>
                            Talent eksternal
                        </label>
