<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl">Kebutuhan: {{ $event->name }}</h2>
    </x-slot>
    ...
    <main class="p-6">
        ...
        <form class="mb-6 flex gap-3" method="POST" action="{{ route('admin.kebutuhan-event.store',$event) }}">
            ...
            <input type="number" name="quantity" min="1" class="border rounded px-3 py-2 w-24" placeholder="Qty"
                required>
        ...
        <x-table>
            ...
            <tr>
                ...
                <td class="px-4 py-2">
                    <form method="POST" action="{{ route('admin.kebutuhan-event.update', [$event,$req]) }}"
                        class="flex gap-2">
                        @csrf @method('PUT')
            ...
            @empty
            <tr>
                <td class="px-4 py-4 text-gray-500 italic" colspan="3">Belum ada kebutuhan.</td>
            </tr>
            @endforelse
        </x-table>
    </main>
